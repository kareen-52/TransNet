...
class CreateShipmentCubit extends Cubit<CreateShipmentState> {
  ...
  void nextStep() {
    if (currentStep == 0) {
      if (startGovernorate == null || endGovernorate == null || startLat == null || endLat == null) {
        emit(
          CreateShipmentState.submitError(
            ApiErrorModel(message: "يرجى تحديد المحافظات والمواقع على الخريطة أولاً"),
          ),
        );
        _updateUI();
        return;
  ...
  void getGovernorates({ShipmentModel? shipmentToEdit}) async {
    ...
    result.when(
      success: (govs) {
        if (govs.isEmpty) {
          emit(CreateShipmentState.govError(ApiErrorModel(message: "لا يوجد محافظات متاحة حالياً")));
        } else {
          governorates = govs;
          if (isEditMode && _shipmentToEdit != null) {
            _editData(_shipmentToEdit!);
  ...
  void submitShipment() async {
    ...
    if (isEditMode && _shipmentToEdit != null) {
      ...
      if (double.parse(weightController.text) != _shipmentToEdit!.weight) changedFields['weight'] = double.parse(weightController.text);
      if (double.parse(heightController.text) != _shipmentToEdit!.height) changedFields['height'] = double.parse(heightController.text);
      if (double.parse(widthController.text) != _shipmentToEdit!.width) changedFields['width'] = double.parse(widthController.text);
      if (double.parse(lengthController.text) != _shipmentToEdit!.length) changedFields['length'] = double.parse(lengthController.text);
      if (objectController.text != _shipmentToEdit!.object) changedFields['object'] = objectController.text;
      if (insurance != _shipmentToEdit!.insurance) changedFields['insurance'] = insurance;
      if (startLat.toString() != _shipmentToEdit!.startPositionLat.toString()) changedFields['start_position_lat'] = startLat;
      if (startLng.toString() != _shipmentToEdit!.startPositionLng.toString()) changedFields['start_position_lng'] = startLng;
      if (endLat.toString() != _shipmentToEdit!.endPositionLat.toString()) changedFields['end_position_lat'] = endLat;
      if (endLng.toString() != _shipmentToEdit!.endPositionLng.toString()) changedFields['end_position_lng'] = endLng;
      if (startGovernorate!.id.toString() != _shipmentToEdit!.startGovernorateId.toString()) changedFields['start_governorate_id'] = startGovernorate!.id;
      if (endGovernorate!.id.toString() != _shipmentToEdit!.endGovernorateId.toString()) changedFields['end_governorate_id'] = endGovernorate!.id;

      if (changedFields.isEmpty) {
        emit(const CreateShipmentState.submitSuccess("لم تقم بتعديل أي بيانات"));
        return;
      }

      final result = await _repo.updateShipment(changedFields);
      if (isClosed) return;
      result.when(
        success: (message) => emit(CreateShipmentState.submitSuccess(message)),
        failure: (error) => emit(CreateShipmentState.submitError(error)),
