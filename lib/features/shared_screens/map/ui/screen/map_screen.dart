...
import 'package:graduation_progect/core/networking/cached_network_tile_provider.dart';
...
import 'package:graduation_progect/features/shared_screens/map/ui/widgets/map_offline_badge.dart';
import 'package:graduation_progect/features/shared_screens/map/ui/widgets/map_shimmer_placeholder.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  bool _isOffline = !ConnectivityHelper.isOnline;
  ...
  void initState() {
    super.initState();
    _initLocation();
    _connectivitySub = ConnectivityHelper.onConnectivityChange.listen((isOnline) {
  ...
  Future<void> _initLocation() async {
    ...
    if (mounted) setState(() => _currentLoc = data);
  ...
  Future<void> _fetchRoute() async {
    ...
    if (_isOffline) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('رسم المسار يتطلب اتصالاً بالإنترنت')),
      );
      return;
    }
          ...
          : Stack(
              children: [
                FlutterMap(
                  ...
                  options: MapOptions(
                    initialCenter: LatLng(_currentLoc!.latitude!, _currentLoc!.longitude!),
                    initialZoom: 15,
                    onTap: (_, p) {
                      if (_distance == null) {
                        setState(() { _destination = p; _searchController.clear(); });
                      }
                    },
                  ),
                  children: [
                    TileLayer(
                      ...
                      tileProvider: CachedNetworkTileProvider(isOffline: _isOffline),
                      panBuffer: 2,
                      keepBuffer: 4,
                    ),
                ...
                if (_isOffline)
                  const Positioned(
                    ...
                    left: 0,
                    right: 0,
                    child: Center(child: MapOfflineBadge()),
                  ),

                
                if (_distance == null && !_isOffline)
                  Positioned(
                    top: 50,
                    left: 15,
                    right: 15,
                ...
                Positioned(
                  bottom: _distance != null ? (_isSheetMinimized ? 100 : 250) : 100,
                  right: 20,
                ...
                if (_destination != null && _distance == null && !_isOffline)
                  Positioned(
                    bottom: 30,
                    left: 20,
                    right: 20,
                  ...
                  Align(
                    ...
                    child: _isSheetMinimized
                        ...
                        : BottomInfoSheet(
                            ...
                            duration: _duration!,
                            onCancel: _reset,
                            onMinimize: () => setState(() => _isSheetMinimized = true),
                            onStart: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text("بدأت الرحلة.. جاري التتبع")),
                              );
                            },
                          ),
                  ),
  ...
  Widget _buildMinimizedInfo() {
    return Padding(
      ...
      child: FloatingActionButton.extended(
        ...
        label: Text("${_distance!.toStringAsFixed(1)} كم - ${_duration!.round()} دقيقة"),
  ...
  Widget _buildUserMarker() {
    return Container(
      ...
      child: Center(child: Icon(Icons.circle, color: Colors.blue[700], size: 18)),
    );
  }
}
